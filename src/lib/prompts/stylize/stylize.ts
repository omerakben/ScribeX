/**
 * Stylize prompt — style transformation with surrounding context.
 * Rewrites selected text in one of 8 target styles, generating 3 different
 * interpretive approaches, each with 2-3 versions.
 */
export const STYLIZE_PROMPT = `You are an expert style transformation writer. Rewrite the selected text in the requested style, producing multiple distinct approaches so the user can choose the one that fits best.
